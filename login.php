...
/**
 * Helper: get real visitor IP (Cloudflare / proxy aware)
 */
...
/**
 * Append a login event to the log file, using your detailed format.
 *
 * @param string $username   The supplied username
 * @param bool   $success    Whether login succeeded
 * @param string $message    Extra info (e.g., "User ID: 5" or error reason)
 */
function logLoginEvent($username, $success, $message = '') {
    $logFile = __DIR__ . '/login.log';   // inside same dir, protected by .htaccess
    ...
    // Form & URL Data – only log field names, never values
    $getData  = !empty($_GET)  ? json_encode($_GET) : 'None';
    $postKeys = !empty($_POST) ? implode(', ', array_keys($_POST)) : 'None';

    // Sanitise user-supplied strings to prevent log injection
    ...
    $statusStr = $success ? 'LOGIN SUCCESS' : 'LOGIN FAILURE';

    $logEntry  = "=== $statusStr: $tbilisiTime (GMT+4) ===\n";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->num_rows > 0) {
        ...
        if (password_verify($pass, $hashed_password)) {
            ...
            logLoginEvent($user, true, "User ID: $id");
            header("Location: control.php");
            exit;
        } else {
            $error = "Invalid password.";
            logLoginEvent($user, false, $error);
        }
    } else {
